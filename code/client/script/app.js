...
function find (q, t, n=-1, firstChar=null)
{
	...
	else
	{
	    ...
	    {
		loadItem(`${booksPath}/${c}/list`, function (list) {
...
function O (path)
{
	...
	item = item.replace(/\n/g, "<br>");
	item = item.replace(
	    /\[map_([^\]]+)\]/, "<button type='button' \
class='icon mapBtn' onclick='map(\"$1\")'>search</button>\
<div id='mapRes'></div>");
	DRes.innerHTML = item;
	applyTheme(currentTheme);
    });
}
function loadItem (path, callback, updCallback=(x)=>null, verPath=versionBooksPath)
{
...
function downloadItem (path, callback, verPath=versionBooksPath)
{
	...
	lockedGetUrl(verPath, function (x) {
	    if(x === null)
	    {
		callback(null);
		return;
...
function plan ()
{
	...
	if(!objs) return null;
	let text = "", table = "";
	try {text = "<p>" + objs.text.join("<br>") + "</p>";} catch (e) {}
	try {
...
function applyTheme (theme)
{
    ...
    try {mapRes.style.background = foreLight;} catch (e) {}
    ...
    D.querySelectorAll(".mapBtn").forEach(function (o) {
	o.style.color = key;
    });
}
...
window.addEventListener("load", function () {
	...
	let verObj = x.responseText.split("\n");
